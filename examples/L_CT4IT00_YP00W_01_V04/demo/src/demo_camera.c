...
#define DEMO_CAMERA_SDA_PIN		255                  
#define DEMO_CAMERA_SCL_PIN		255                  
...
static char txbuf[sizeof("result: ") + sizeof(decodeResult)] = {0};
...
static void liot_uart2_notify_cb(liot_uart_e port, char *data, uint32_t size, void *argc)
{
    (void)argc;
    liot_trace("UART port %d receive size:%d, data=%s", port, size, data);
}

static void camdemo_task(void *argv)
{
    int ret;
    ...
    liot_trace("camera demo uart init ret %d", ret);

    img = liot_rtos_malloc(IMG_WIDTH*IMG_HEIGHT*2);  
    if (img == NULL)
    {
        liot_trace("camera demo image malloc fail");
        return;
    }
    ...
    while(1)  
    {
        ...
        if(liot_image_decoder(img, IMG_WIDTH, IMG_HEIGHT) == LIOT_DECODER_SUCCESS)  
        {
            ...
            decodeResult[sizeof(decodeResult) - 1] = '\0';
            liot_trace("result: %s", decodeResult);  
            snprintf(txbuf, sizeof(txbuf), "result: %s", (char *)decodeResult);  
            Liot_UartSend(L_UART1, (unsigned char *)txbuf, strlen(txbuf));  
        }
    }
}

static void Decoder_task_start(void)
{
    camdemo_task_stack = liot_rtos_malloc(DEMO_DECODER_TASK_STACK_SIZE);
    if (camdemo_task_stack == NULL)
    {
        liot_trace("camera demo stack malloc fail");
        return;
    }
...
{
    (void)argv;
    liot_trace("liot_camera_demo_thread start");
    /**< Start decoder task */
    Decoder_task_start();
    while(1)
    {
