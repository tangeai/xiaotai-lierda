...
#define DEMO_TIRTC_SEND_ERROR_LOG_MS  5000U
/*
...
#define DEMO_TIRTC_ERR_ROUTE          (-2006)

/* These public wire/ABI fields have explicit widths, independent of the
 * archive's small-enum tag. Never pass a host-sized structure to the MCU. */
typedef char runtime_frame_abi_size[(sizeof(TIRTCFRAMEINFO) == 12U) ? 1 : -1];
#if UINTPTR_MAX == UINT32_MAX
typedef char runtime_callback_abi_size[(sizeof(TIRTCCALLBACKS) == 60U) ? 1 : -1];
#endif
static liot_task_t s_service_task;
static bool s_service_reserved;
static int s_service_failure_stage, s_service_failure_result;
static tirtc_network_snapshot_t s_runtime_route;
static bool s_runtime_route_valid;
static uint32_t s_runtime_binding_generation;
static uint32_t s_runtime_credential_epoch;

static bool runtime_route_matches(void)
{
    tirtc_network_snapshot_t route, started;
    demo_binding_snapshot_t binding;
    uint32_t binding_generation, credential_epoch;
    bool valid;
    tirtc_network_get_snapshot(&route);
    demo_binding_get_snapshot(&binding);
    liot_rtos_enter_critical();
    started = s_runtime_route;
    valid = s_runtime_route_valid;
    binding_generation = s_runtime_binding_generation;
    credential_epoch = s_runtime_credential_epoch;
    liot_rtos_exit_critical();
    return valid && route.ready && route.time_valid &&
           binding.state == DEMO_BIND_BOUND && binding.generation == binding_generation &&
           credential_epoch != 0U && binding.credential_epoch == credential_epoch &&
           route.generation == started.generation && route.sim_id == started.sim_id;
}

/* A terminal start error belongs to the identity/route used for that attempt.
 * Do not retry unchanged bad credentials, but do not poison a later binding
 * or SIM/PDP generation either. This accessor performs no modem I/O. */
static bool runtime_start_context_changed(void)
{
    ...
    demo_binding_get_snapshot(&binding);
    liot_rtos_enter_critical();
    changed = s_runtime_route_valid &&
        (route.generation != s_runtime_route.generation ||
         route.sim_id != s_runtime_route.sim_id ||
         binding.generation != s_runtime_binding_generation ||
         binding.credential_epoch != s_runtime_credential_epoch);
    liot_rtos_exit_critical();
    return changed;
}
...
/* Protected by the same task critical section as s_connection. Revocation
 * requests cooperative cleanup; it never transfers the SDK handle. */
static bool s_ai_standby_owned;
static bool s_ai_standby_revoked;
...
static uint32_t s_service_users; /* Global SDK calls which have no connection. */
...
/* Context tokens are values, never pointers to reusable callback storage.
 * They survive runtime recovery and never wrap, so a late SDK event cannot
 * acquire a new session's identity even if its opaque handle is reused. */
static uint32_t s_video_monitor_next_token;
static struct
{
    demo_tirtc_owner_e owner;
    uint32_t connection_generation;
...
static bool runtime_dev_connect_needs_cleanup(void)
{
    ...
    /* These audited archives hide the failed handle without freeing it.
     * 9088239c retains the same native DEV connect/error/cleanup code.
    ...
    return build != NULL &&
           ((strstr(build, "v2.4.0-7f41aae1") != NULL &&
            ...
            (strstr(build, "v2.5.0-9088239c") != NULL &&
             strstr(build, "f72f5d3c") != NULL));
}
...
/* One global window bounds repeated socket failures even when several SDK
 * tasks report them. Counters saturate rather than silently wrapping; the
 * summary explicitly identifies that case. No logger/formatter runs locked. */
static struct {
    uint32_t last_report_ms, errors, total, suppressed_total;
    bool reported;
} s_transport_send_errors;
...
static void tirtc_transport_log_callback(const char *format, va_list args)
{
    ...
    if (written > 0)
    {
        ...
        if (!runtime_log_is_sensitive(line, output_length))
        {
            if (strstr(line, "send_data_with_retry") != NULL &&
                strstr(line, "fatal error") != NULL)
            {
                uint32_t errors, total, suppressed, now;
                bool report;

                liot_rtos_enter_critical();
                /* F6D_A's time API only reads the RTOS tick (including when
                 * interrupts are masked). Read here to avoid an older task's
                 * timestamp being mistaken for wrap after another report. */
                now = liot_rtos_get_running_time();
                if (s_transport_send_errors.errors != UINT32_MAX)
                    ++s_transport_send_errors.errors;
                if (s_transport_send_errors.total != UINT32_MAX)
                    ++s_transport_send_errors.total;
                report = !s_transport_send_errors.reported ||
                    (uint32_t)(now - s_transport_send_errors.last_report_ms) >=
                        DEMO_TIRTC_SEND_ERROR_LOG_MS;
                if (!report && s_transport_send_errors.suppressed_total != UINT32_MAX)
                    ++s_transport_send_errors.suppressed_total;
                errors = s_transport_send_errors.errors;
                total = s_transport_send_errors.total;
                suppressed = s_transport_send_errors.suppressed_total;
                if (report)
                {
                    s_transport_send_errors.reported = true;
                    s_transport_send_errors.last_report_ms = now;
                    s_transport_send_errors.errors = 0U;
                }
                liot_rtos_exit_critical();
                if (!report)
                    return;
                /* Include this printed failure in errors; preserve the last
                 * quiet interval's suppressed count until the next failure. */
                syslogPrintf("[TIRTC-TRP] send_failures errors_since_report=%lu total=%lu suppressed_total=%lu counts_saturated=%u\r\n",
                             (unsigned long)errors, (unsigned long)total,
                             (unsigned long)suppressed,
                             (errors == UINT32_MAX || total == UINT32_MAX ||
                              suppressed == UINT32_MAX) ? 1U : 0U);
            }
...
/*
 * The pinned libTiRTC.a v2.5.0 native sha256_hmac object passes constant 9,
 * matching the newer mbedTLS enum where
...
/* Video is delivered only while a managed active reference is held. Unknown
 * handles never use the legacy audio broadcast path. */
static void runtime_owner_on_video(demo_tirtc_owner_e owner,
                                   tirtc_conn_t hconn,
                                   const TIRTCFRAMEINFO *frame, void *data)
{
    const demo_tirtc_listener_t *feature = runtime_owner_feature_snapshot(owner);
    const demo_tirtc_incoming_listener_t *incoming = runtime_owner_incoming_snapshot(owner);
    if (feature != NULL && feature->on_video != NULL)
        feature->on_video(hconn, frame, data);
    else if (incoming != NULL && incoming->on_video != NULL)
        incoming->on_video(hconn, frame, data);
}

static int runtime_owner_on_subscribe_video(demo_tirtc_owner_e owner,
                                            tirtc_conn_t hconn, uint8_t stream_id)
{
    const demo_tirtc_listener_t *feature = runtime_owner_feature_snapshot(owner);
    const demo_tirtc_incoming_listener_t *incoming = runtime_owner_incoming_snapshot(owner);
    if (feature != NULL && feature->on_subscribe_video != NULL)
        return feature->on_subscribe_video(hconn, stream_id);
    if (incoming != NULL && incoming->on_subscribe_video != NULL)
        return incoming->on_subscribe_video(hconn, stream_id);
    return TIRTC_E_INVALID_HANDLE;
}

static void runtime_owner_on_unsubscribe_video(demo_tirtc_owner_e owner,
                                               tirtc_conn_t hconn, uint8_t stream_id)
{
    const demo_tirtc_listener_t *feature = runtime_owner_feature_snapshot(owner);
    const demo_tirtc_incoming_listener_t *incoming = runtime_owner_incoming_snapshot(owner);
    if (feature != NULL && feature->on_unsubscribe_video != NULL)
        feature->on_unsubscribe_video(hconn, stream_id);
    else if (incoming != NULL && incoming->on_unsubscribe_video != NULL)
                                           ...
                                           uint32_t generation)
{
    ...
    liot_rtos_enter_critical();
    if (runtime_route_matches() && s_connection.owner == owner &&
                                            ...
                                            bool *was_standby)
{
    ...
    liot_rtos_enter_critical();
    /* Keep standby traffic out of the legacy broadcast fallback as well:
     * route loss or an error can retire active before its final data callback. */
    *was_standby = s_ai_standby_owned &&
        s_connection.owner == DEMO_TIRTC_OWNER_AI && hconn != NULL &&
        ((hconn == s_connection.active &&
          s_connection.active_generation == s_connection.session_generation) ||
         (hconn == s_connection.closing &&
          s_connection.closing_generation == s_connection.session_generation) ||
         (hconn == s_connection.pending_handle &&
          s_connection.pending_generation == s_connection.session_generation));
    if (runtime_route_matches() && hconn != NULL && hconn == s_connection.active &&
                                           ...
                                           void *user_data)
{
    ...
    if (matching)
    {
        ...
        if (callback_error == 0 && hconn != NULL &&
            ...
            s_connection.users == 0U && !s_restart_required && runtime_route_matches())
        {
...
static void runtime_on_conn_error(tirtc_conn_t hconn, int error)
{
    ...
    /* An incoming handshake can fail before on_conn_accepted gives any
     * feature ownership.  on_conn_error still carries a live SDK handle:
     * the SDK requires Disconnect and frees it only after on_disconnected.
     * Preserve listener notification, then reserve deferred cleanup just as
     * for a rejected incoming connection.  The fixed reject table coalesces
     * duplicate errors and an already queued rejection of the same handle. */
    liot_trace("[TIRTC-ADAPTER] unowned connection error handle=%p error=%d; "
               "deferred cleanup\r\n", hconn, error);
    runtime_reject_later(hconn);
}
                             ...
                             void *data)
{
    ...
    if (runtime_active_callback_acquire(hconn, &owner, &was_standby))
    {
        if (!was_standby)
        {
                             ...
                             void *data)
{
    demo_tirtc_owner_e owner = DEMO_TIRTC_OWNER_NONE;
    bool was_standby = false;
    if (frame == NULL || data == NULL || frame->length == 0U) return;
    if (!runtime_active_callback_acquire(hconn, &owner, &was_standby)) return;
    if (!was_standby) runtime_owner_on_video(owner, hconn, frame, data);
    runtime_active_release(hconn);
}
                                      ...
                                      uint8_t stream_id)
{
    ...
    {
        if (!was_standby)
            accepted = runtime_owner_on_subscribe_video(owner, hconn, stream_id);
        runtime_active_release(hconn);
    }
    return accepted;
}
                                         ...
                                         uint8_t stream_id)
{
    ...
    if (!runtime_active_callback_acquire(hconn, &owner, &was_standby)) return;
    if (!was_standby)
        runtime_owner_on_unsubscribe_video(owner, hconn, stream_id);
    runtime_active_release(hconn);
}
...
bool demo_tirtc_admission_try_enter(void)
{
    ...
    liot_rtos_enter_critical();
    if (s_ai_standby_owned)
    {
        s_ai_standby_revoked = true;
    }
    else if (s_ready && !s_restart_required && !s_admission_busy &&
...
{
    if ((unsigned)feature >= (unsigned)DEMO_TIRTC_FEATURE_COUNT)
    {
        return;
    }
    liot_rtos_enter_critical();
    s_feature_listeners[feature] = listener;
    if (feature != DEMO_TIRTC_FEATURE_AI && listener != NULL &&
        s_ai_standby_owned)
    {
        s_ai_standby_revoked = true;
    }
    ...
    const demo_tirtc_incoming_listener_t *listener)
{
    ...
    if (listener != NULL && s_ai_standby_owned)
    {
        s_ai_standby_revoked = true;
    }
    liot_rtos_exit_critical();
...
/* Caller holds task critical throughout every predicate and state commit.
 * Cached route/binding accessors use only nested short critical sections. */
static bool runtime_standby_listeners_clear_locked(void)
{
    unsigned feature;
    if (s_incoming_listener != NULL) return false;
    for (feature = 0; feature < (unsigned)DEMO_TIRTC_FEATURE_COUNT; ++feature)
    {
        if (feature != (unsigned)DEMO_TIRTC_FEATURE_AI &&
            s_feature_listeners[feature] != NULL) return false;
    }
    return true;
}

static bool runtime_session_idle_locked(void)
{
    return s_connection.owner == DEMO_TIRTC_OWNER_NONE &&
           s_connection.active == NULL && s_connection.closing == NULL &&
           !s_connection.connect_request_pending &&
           !s_connection.connect_callback_pending &&
           s_connection.expected_owner == DEMO_TIRTC_OWNER_NONE &&
...
static bool runtime_standby_valid_locked(uint32_t generation)
{
    return generation != 0U && s_ai_standby_owned && !s_ai_standby_revoked &&
           s_connection.owner == DEMO_TIRTC_OWNER_AI &&
           s_connection.session_generation == generation &&
           s_ready && !s_restart_required && !s_admission_busy &&
           runtime_standby_listeners_clear_locked() && runtime_route_matches();
}
...
bool demo_tirtc_ai_standby_allowed(void)
{
    bool allowed;
    liot_rtos_enter_critical();
    allowed = s_ready && !s_restart_required && runtime_session_idle_locked() &&
              runtime_standby_listeners_clear_locked() && runtime_route_matches();
    liot_rtos_exit_critical();
    return allowed;
}

bool demo_tirtc_ai_standby_valid(uint32_t generation)
{
    bool valid;
    liot_rtos_enter_critical();
    valid = runtime_standby_valid_locked(generation);
    liot_rtos_exit_critical();
    return valid;
}

int demo_tirtc_ai_standby_promote(uint32_t generation)
{
    int ret = TIRTC_E_BUSY;
    if (generation == 0U) return TIRTC_E_INVALID_PARAMETER;
    liot_rtos_enter_critical();
    if (s_connection.owner != DEMO_TIRTC_OWNER_AI ||
        s_connection.session_generation != generation || !s_ai_standby_owned)
        ret = TIRTC_E_INVALID_HANDLE;
    else if (!s_ready || s_restart_required || !runtime_route_matches())
        ret = TIRTC_E_NOT_INITIALIZED;
    else if (runtime_standby_valid_locked(generation))
    {
        s_ai_standby_owned = s_ai_standby_revoked = false;
        ret = 0;
    }
    liot_rtos_exit_critical();
    return ret;
...
static int runtime_session_claim(demo_tirtc_owner_e owner,
                                 uint32_t *session_generation, bool standby)
{
    int ret = TIRTC_E_BUSY;
    ...
    *session_generation = 0U;
    liot_rtos_enter_critical();
    if (!standby && owner != DEMO_TIRTC_OWNER_AI && s_ai_standby_owned)
    {
        s_ai_standby_revoked = true;
    }
    else if (!s_ready || s_restart_required || !runtime_route_matches())
    ...
    else if (runtime_session_idle_locked() &&
             (!standby || runtime_standby_listeners_clear_locked()))
    {
        ...
        *session_generation = s_connection.session_generation;
        s_ai_standby_owned = standby;
        s_ai_standby_revoked = false;
        ret = 0;
    }
    liot_rtos_exit_critical();
    if (ret == 0)
...
int demo_tirtc_session_claim(demo_tirtc_owner_e owner,
                             uint32_t *session_generation)
{
    return runtime_session_claim(owner, session_generation, false);
}

int demo_tirtc_ai_standby_claim(uint32_t *session_generation)
{
    return runtime_session_claim(DEMO_TIRTC_OWNER_AI, session_generation, true);
}
                               ...
                               uint32_t session_generation)
{
    ...
    liot_rtos_enter_critical();
    if (!s_ready || s_restart_required || !runtime_route_matches())
    {
        ret = TIRTC_E_NOT_INITIALIZED;
    }
    else if (s_connection.owner != owner ||
                                  ...
                                  void *user_data)
{
    ...
    /* valid() may have succeeded before another feature revoked standby.
     * Recheck at the same atomic reservation boundary as every connect. */
    if (s_ai_standby_owned && !runtime_standby_valid_locked(session_generation))
    {
        liot_rtos_exit_critical();
        return TIRTC_E_BUSY;
    }
...
static tirtc_conn_t runtime_video_acquire(demo_tirtc_owner_e owner,
                                         uint32_t session_generation)
{
    tirtc_conn_t hconn = NULL;
    /* Admission and borrowing are atomic with standby promotion/revocation. */
    liot_rtos_enter_critical();
    if (!s_ai_standby_owned)
        hconn = runtime_active_acquire(owner, session_generation);
    liot_rtos_exit_critical();
                         ...
                         const TIRTCFRAMEINFO *frame, const void *data)
{
    tirtc_conn_t hconn;
    int ret;
    if (frame == NULL || data == NULL || frame->length == 0U)
        return TIRTC_E_INVALID_PARAMETER;
    hconn = runtime_video_acquire(owner, session_generation);
    if (hconn == NULL) return TIRTC_E_INVALID_HANDLE;
    ret = TiRtcSendVideoStream(hconn, frame, data);
    runtime_active_release(hconn);
    return ret;
}
...
{
    tirtc_conn_t hconn = runtime_video_acquire(owner, session_generation);
    int ret;
    if (hconn == NULL) return TIRTC_E_INVALID_HANDLE;
    ret = TiRtcSubscribeVideo(hconn, stream_id);
    runtime_active_release(hconn);
    return ret;
}
                                ...
                                uint32_t session_generation, uint8_t stream_id)
{
    ...
    int ret;
    if (hconn == NULL) return TIRTC_E_INVALID_HANDLE;
    ret = TiRtcUnsubscribeVideo(hconn, stream_id);
    runtime_active_release(hconn);
    return ret;
}
...
/* Called with the runtime critical section held. Route snapshots are bounded
 * RAM reads, as in runtime_active_acquire; no SDK operation is performed. */
static bool runtime_video_monitor_current(void)
{
    return !s_ai_standby_owned && s_connection.active != NULL &&
        s_connection.owner == s_video_monitor.owner &&
        s_connection.active_owner == s_video_monitor.owner &&
        s_connection.session_generation == s_video_monitor.stats.generation &&
        s_connection.active_generation == s_video_monitor.stats.generation &&
        s_connection.connection_generation == s_video_monitor.connection_generation &&
        runtime_route_matches();
}

static void runtime_video_send_event(const tgtrp_video_send_event_t *event,
                                     void *context)
{
    uint32_t now;
    const uintptr_t token = (uintptr_t)context;
    bool has_bwe;
    if (event == NULL || token == 0U ||
        event->struct_size < offsetof(tgtrp_video_send_event_t, target_bitrate_bps))
        return;
    now = liot_rtos_get_running_time();
    has_bwe = event->struct_size >=
        offsetof(tgtrp_video_send_event_t, bwe_state);
    liot_rtos_enter_critical();
    if (token == s_video_monitor.token &&
        (s_video_monitor.registering || s_video_monitor.stats.registered) &&
        event->stream_id == s_video_monitor.stats.stream_id &&
        runtime_video_monitor_current())
    {
        demo_tirtc_video_send_stats_t *stats = &s_video_monitor.stats;
        stats->available = true;
        stats->event_at_ms = now;
        stats->event_seq = event->event_seq;
        stats->event_type = event->event_type;
        stats->reason = event->reason;
        stats->rtt_ms = event->rtt_ms;
        stats->unacked_segments = event->unacked_segments;
        stats->pending_frames = event->pending_frames;
        stats->pending_wait_ms = event->pending_head_wait_ms;
        stats->dropped_pending_frames = event->dropped_pending_frames;
        stats->receive_bitrate_bps = has_bwe ? event->receive_bitrate_bps : 0U;
        stats->loss_rate_ppm = has_bwe ? event->loss_rate_ppm : 0U;
        stats->loss_available = has_bwe && event->loss_rate_ppm <= 1000000U &&
            (event->reason == TGTRP_VIDEO_SEND_EVENT_REASON_RECEIVER_BWE ||
             event->reason == TGTRP_VIDEO_SEND_EVENT_REASON_RECEIVER_BWE_RECOVERY);
    }
    liot_rtos_exit_critical();
}
                                        ...
                                        uint8_t stream_id)
{
    ...
    uint32_t token = 0U;
    int ret = TIRTC_E_INVALID_HANDLE;
    if ((owner != DEMO_TIRTC_OWNER_WECHAT && owner != DEMO_TIRTC_OWNER_DEV_CHAT &&
         owner != DEMO_TIRTC_OWNER_LIVE) ||
        session_generation == 0U || stream_id > 15U)
        return TIRTC_E_INVALID_PARAMETER;
    hconn = runtime_video_acquire(owner, session_generation);
    if (hconn == NULL) return TIRTC_E_INVALID_HANDLE;
    liot_rtos_enter_critical();
    /* A disconnect may detach the active handle just after the borrow. */
    if (s_connection.active == hconn && runtime_route_matches())
    {
        if (s_video_monitor.stats.attempted && s_video_monitor.owner == owner &&
            s_video_monitor.stats.generation == session_generation &&
            s_video_monitor.connection_generation == s_connection.connection_generation)
        {
            /* TiRTC stores one callback/context pair per connection; this
             * adapter deliberately monitors only its single video stream. */
            ret = s_video_monitor.stats.stream_id == stream_id ?
                s_video_monitor.stats.registration_result : TIRTC_E_INVALID_PARAMETER;
        }
        else if (s_video_monitor_next_token == UINT32_MAX)
        {
            ret = TIRTC_E_BUSY;
        }
        else
        {
            token = ++s_video_monitor_next_token;
            memset(&s_video_monitor, 0, sizeof(s_video_monitor));
            s_video_monitor.owner = owner;
            s_video_monitor.connection_generation = s_connection.connection_generation;
            s_video_monitor.token = token;
            s_video_monitor.registering = true;
            s_video_monitor.stats.attempted = true;
            s_video_monitor.stats.generation = session_generation;
            s_video_monitor.stats.stream_id = stream_id;
            s_video_monitor.stats.registration_result = TIRTC_E_BUSY;
        }
    }
    liot_rtos_exit_critical();
    if (token != 0U)
    {
        ret = TiRtcSetVideoSendMonitor(hconn, stream_id, runtime_video_send_event,
                                       (void *)(uintptr_t)token);
        liot_rtos_enter_critical();
        if (s_video_monitor.token == token)
        {
            s_video_monitor.registering = false;
            s_video_monitor.stats.registration_result = ret;
            s_video_monitor.stats.registered = ret == 0;
            if (ret != 0)
            {
                s_video_monitor.stats.available = false;
                s_video_monitor.stats.loss_available = false;
            }
        }
        liot_rtos_exit_critical();
    }
                                   ...
                                   demo_tirtc_video_send_stats_t *stats)
{
    ...
    uint32_t now;
    int ret = 0;
    if (stats == NULL) return TIRTC_E_INVALID_PARAMETER;
    ...
    if (s_connection.active != hconn || !runtime_route_matches())
        ret = TIRTC_E_INVALID_HANDLE;
    else if (s_video_monitor.stats.attempted && runtime_video_monitor_current())
        *stats = s_video_monitor.stats;
    liot_rtos_exit_critical();
    /* Read the clock after the copy, so an event from a concurrent callback
     * cannot have a timestamp newer than the age calculation's clock sample. */
    now = liot_rtos_get_running_time();
    if (stats->available)
    {
        stats->age_ms = now - stats->event_at_ms;
        stats->stale = stats->age_ms > 10000U;
    }
    runtime_active_release(hconn);
    return ret;
}
...
{
    return s_ready && !s_restart_required && runtime_route_matches();
}
...
typedef struct {
    ...
    void *user;
} runtime_service_context_t;

static void runtime_service_callback(const char *body, void *user)
{
    runtime_service_context_t *request = (runtime_service_context_t *)user;
    bool current;
    liot_rtos_enter_critical();
    current = s_ready && !s_restart_required && runtime_route_matches() &&
              demo_binding_service_context_current(&request->context);
    liot_rtos_exit_critical();
    /* The callback is synchronous in pinned libTiRTC. The enclosing request
     * retains the SDK lease, including while application callback runs. */
    if (current && request->callback != NULL) request->callback(body, request->user);
}
    ...
    void *user)
{
    int result;
    bool current;
    runtime_service_context_t request;
    if (context == NULL || path == NULL || body == NULL ||
        strncmp(path, "/v1/", 4U) != 0 || strlen(path) > 127U ||
        strlen(body) >= 2048U)
        return TIRTC_E_INVALID_PARAMETER;
    liot_rtos_enter_critical();
    if (!s_ready || s_restart_required || !runtime_route_matches() ||
        !demo_binding_service_context_current(context))
    {
        liot_rtos_exit_critical();
        return DEMO_TIRTC_ERR_ROUTE;
    }
    if (s_service_users != 0U)
    {
        liot_rtos_exit_critical();
        return TIRTC_E_BUSY;
    }
    ...
    request.user = user;
    liot_rtos_exit_critical();
    result = TiRtcServiceRequest(path, body, NULL,
                                callback != NULL ? runtime_service_callback : NULL,
    ...
    liot_rtos_exit_critical();
    runtime_signal();
    return current ? result : DEMO_TIRTC_ERR_ROUTE;
}
...
static void runtime_start_until_terminal(void)
{
    ...
    tirtc_network_snapshot_t route, after;
    demo_binding_snapshot_t binding, binding_after;
    int ret;
    ...
    while (!s_ready && !s_restart_required)
    {
        ...
        for (;;)
        {
            ...
            tirtc_network_get_snapshot(&route);
            demo_binding_get_snapshot(&binding);
            if (route.ready && route.time_valid && binding.state == DEMO_BIND_BOUND &&
                demo_binding_get_tirtc_identity(&identity) == 0)
            {
                tirtc_network_get_snapshot(&after);
                demo_binding_get_snapshot(&binding_after);
                if (after.ready && after.time_valid &&
                    binding_after.state == DEMO_BIND_BOUND && binding_after.generation == binding.generation &&
                    binding.credential_epoch != 0U && binding_after.credential_epoch == binding.credential_epoch &&
                    after.generation == route.generation && after.sim_id == route.sim_id)
                    break;
            }
            memset(&identity, 0, sizeof(identity));
        ...
        liot_rtos_enter_critical();
        s_runtime_route = route;
        s_runtime_binding_generation = binding.generation;
        s_runtime_credential_epoch = binding.credential_epoch;
        s_runtime_route_valid = true;
        liot_rtos_exit_critical();
...
/* Retry diagnostics belong to the service lifecycle, not each allocation.
 * A task-create success is not recovery until its queue/semaphore exist. */
static bool runtime_service_failure_changed(int stage, int result)
{
    bool changed;
    liot_rtos_enter_critical();
    changed = s_service_failure_stage != stage || s_service_failure_result != result;
    s_service_failure_stage = stage; s_service_failure_result = result;
    liot_rtos_exit_critical();
    return changed;
}
...
void demo_tirtc_task(void *argv)
{
    ...
    s_accept_callbacks_pending = 0U;
    if (s_runtime_sem == NULL &&
        liot_rtos_semaphore_create(&s_runtime_sem, 0U) != LIOT_OSI_SUCCESS)
    {
        goto init_failed;
    }
    if (s_reject_queue == NULL && liot_rtos_queue_create(&s_reject_queue, sizeof(tirtc_conn_t),
    ...
    if (runtime_service_failure_changed(0, 0))
        liot_trace("[TIRTC] adapter task/queue/semaphore startup recovered\r\n");
    liot_trace("[TIRTC] minimal adapter waiting for provisioned identity\r\n");
    runtime_start_until_terminal();

    while (1)
    {
        if ((s_ready && !runtime_route_matches()) ||
            (!s_ready && !s_restart_required &&
             runtime_start_context_changed()))
            demo_tirtc_require_restart(DEMO_TIRTC_ERR_ROUTE);
...
init_failed:
    /* F6D_A semaphore_delete takes an empty semaphore forever. These are
     * process-lifetime objects; keep any successful allocation for retry. */
    s_error = DEMO_TIRTC_ERR_INIT;
    if (runtime_service_failure_changed(2, DEMO_TIRTC_ERR_INIT))
        liot_trace("[TIRTC] adapter queue/semaphore init failed\r\n");
    liot_rtos_enter_critical();
    s_service_task = NULL;
    s_service_reserved = false;
    liot_rtos_exit_critical();
...
int tirtc_runtime_start_service(void)
{
    int result;
    liot_rtos_enter_critical();
    if (s_service_reserved) {
        liot_rtos_exit_critical();
        return 0;
    }
    s_service_reserved = true;
    s_error = 0;
    liot_rtos_exit_critical();
    result = liot_rtos_task_create(&s_service_task, 64U * 1024U, 10U,
                                   "tirtc_runtime", demo_tirtc_task, NULL);
    if (result != LIOT_OSI_SUCCESS) {
        ...
        s_error = DEMO_TIRTC_ERR_INIT;
        liot_rtos_exit_critical();
        if (runtime_service_failure_changed(1, result))
            liot_trace("[TIRTC] runtime task allocation failed ret=%d\r\n", result);
        return DEMO_TIRTC_ERR_INIT;
    }
    return 0;
}
