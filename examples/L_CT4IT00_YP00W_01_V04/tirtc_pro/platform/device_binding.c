...
#define BIND_SIM_ID                  (s_attempt.sim_id)
...
#define BIND_HTTP_REQUEST_MAX        2048
...
#define BIND_SERVICE_RESPONSE_MAX    8192
...
enum
{
    ...
    BIND_ERR_NETWORK_CHANGED = -130,
    BIND_ERR_SIM_UNSUPPORTED = -131,
};
...
typedef struct
{
    ...
    char response[]; /* one allocation, retained with its async callback context */
} binding_http_ctx_t;
...
static tirtc_network_snapshot_t s_attempt;
static bool s_attempt_valid;
static bool s_binding_required;
static bool s_api_ready;
static uint32_t s_temp_cookie;
static bool s_temp_accept;
static bool s_temp_events_allowed;
static uint32_t s_temp_callbacks;
...
bool tirtc_platform_temp_mqtt_idle(void)
{
    ...
    idle = !s_mqtt_client && !s_mqtt_sem && !s_mqtt_cleanup_pending &&
           !s_mqtt_deinit_submitted && !s_temp_callbacks;
...
{
    tirtc_network_snapshot_t current, attempt;
    bool valid;
    tirtc_network_get_snapshot(&current);
    liot_rtos_enter_critical();
    attempt = s_attempt; valid = s_attempt_valid;
    liot_rtos_exit_critical();
    return valid && current.ready && current.time_valid && current.sim_id == attempt.sim_id &&
...
/* Caller holds the short task critical section. The credential epoch is
 * independent of route generation: unbinding/rebinding the same identity on
 * the same PDP must still invalidate in-flight grants and media sessions. */
static void binding_advance_credential_epoch_locked(void)
{
    if (++s_snapshot.credential_epoch == 0U)
        ++s_snapshot.credential_epoch;
}
...
{
    char safe_code[7] = {0};
    uint32_t sampled_ms = liot_rtos_get_running_time();
    ...
    liot_rtos_enter_critical();
    if (s_snapshot.state == DEMO_BIND_BOUND && state != DEMO_BIND_BOUND)
        binding_advance_credential_epoch_locked();
    s_snapshot.state = state;
    memcpy(s_snapshot.code, safe_code, sizeof(s_snapshot.code));
    s_snapshot.seconds_left = seconds_left;
    s_snapshot.error = error;
    s_snapshot.api_ready = s_api_ready;
    s_snapshot.binding_required = s_binding_required;
    s_snapshot.generation = s_attempt.generation;
    s_snapshot.sample_ms = sampled_ms;
    memset(s_snapshot.device_id, 0, sizeof(s_snapshot.device_id));
    if (s_have_credentials) memcpy(s_snapshot.device_id, s_credentials.device_id, sizeof(s_snapshot.device_id));
    liot_rtos_exit_critical();

    tirtc_platform_binding_event((int)state, error);
...
static void binding_snapshot_set_seconds(uint16_t seconds_left)
{
    ...
    s_snapshot.seconds_left = seconds_left;
    s_snapshot.sample_ms = sampled_ms;
    if (!seconds_left) memset(s_snapshot.code, 0, sizeof(s_snapshot.code));
    liot_rtos_exit_critical();
}
...
bool demo_binding_service_context(demo_binding_service_context_t *out)
{
    ...
    valid = s_snapshot.state == DEMO_BIND_BOUND && s_snapshot.api_ready &&
        s_have_credentials && s_snapshot.credential_epoch != 0U &&
        route.ready && route.time_valid && route.sim_id == 0 &&
        s_snapshot.generation == route.generation &&
        tirtc_platform_attempt_current() && demo_formal_mqtt_is_online();
    if (valid) {
        out->binding_generation = s_snapshot.generation;
        out->credential_epoch = s_snapshot.credential_epoch;
        out->route_generation = route.generation;
        out->sim_id = route.sim_id;
    }
    liot_rtos_exit_critical();
    return valid;
}
...
{
    demo_binding_service_context_t current;
    return context && demo_binding_service_context(&current) &&
        context->credential_epoch == current.credential_epoch &&
        context->binding_generation == current.binding_generation &&
        context->route_generation == current.route_generation &&
        context->sim_id == current.sim_id;
}
...
static bool binding_identifier_valid(const char *text, size_t capacity)
{
    size_t i;
...
static bool binding_secret_valid(const char *text, size_t capacity)
{
    size_t i;
    if (!text || !text[0]) return false;
    for (i = 0; i < capacity; i++) {
...
static bool binding_nvm_record_valid(const binding_nvm_record_t *record)
{
    ...
    if (!binding_identifier_valid(record->device_id, sizeof(record->device_id)) ||
        !binding_secret_valid(record->device_key, sizeof(record->device_key)) ||
                                     ...
                                     const char *device_key)
{
    ...
    if (!binding_identifier_valid(device_id, BIND_DEVICE_ID_MAX) ||
        !binding_secret_valid(device_key, BIND_DEVICE_KEY_MAX) ||
        id_len >= BIND_DEVICE_ID_MAX || key_len >= BIND_DEVICE_KEY_MAX)
    {
        return -1;
    }
...
static int binding_build_auth_headers(char *headers, size_t headers_size)
{
    ...
    headers[0] = '\0';
    if (!tirtc_platform_attempt_current() || !tirtc_platform_utc_seconds(&timestamp_s))
    {
        ret = BIND_ERR_TIME;
        goto cleanup;
    }
    ...
    TIRTC_LOG_DEBUG("[BIND] auth timestamp=%u\r\n",
               (unsigned int)timestamp_s);
                                  ...
                                  void *arg)
{
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] event=%d code=0x%x\r\n",
                                  ...
                                  uint32_t timeout_ms)
{
    ...
    while (!(*flag))
    {
        uint32_t elapsed_ms = liot_rtos_get_running_time() - started_ms;
        uint32_t remaining_ms;
        if (elapsed_ms >= timeout_ms) break;
        if (!tirtc_platform_attempt_current()) return -1;
        remaining_ms = timeout_ms - elapsed_ms;
        /* A short sleep can resume much later under modem/UI load. Count
         * actual monotonic ticks, including across the uint32_t wrap. */
        liot_rtos_task_sleep_ms(remaining_ms < BIND_HTTP_WAIT_STEP_MS
                                   ? remaining_ms : BIND_HTTP_WAIT_STEP_MS);
    }
    return *flag ? 0 : -1;
}
...
{
    uint32_t started_ms = liot_rtos_get_running_time();
    uint32_t last_retry_ms = started_ms;
    bool stop_retried = false;
    ...
    while (!ctx->closed)
    {
        ...
        uint32_t remaining_ms;
        if (elapsed_ms > 0U && elapsed_ms >= timeout_ms) break;
        ...
        if (ctx->open_failed && (!stop_retried ||
            (uint32_t)(now_ms - last_retry_ms) >= BIND_HTTP_REAPER_RETRY_MS))
        {
            s_http_queue_reset_guard = true;
            (void)liot_httpc_stop(&ctx->client);
            last_retry_ms = liot_rtos_get_running_time();
            stop_retried = true;
        }
        elapsed_ms = liot_rtos_get_running_time() - started_ms;
        if (ctx->closed || elapsed_ms >= timeout_ms)
        {
            break;
        }
        remaining_ms = timeout_ms - elapsed_ms;
...
static void binding_http_retained_cleanup_complete(binding_http_ctx_t *ctx)
{
    ...
    memset(ctx, 0, sizeof(*ctx)+(size_t)ctx->response_capacity);
    liot_rtos_free(ctx);
}
...
static void binding_http_reaper_task(void *arg)
{
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] reaper waiting for close\r\n");
    while (!ctx->closed)
    {
    ...
    for (;;)
    {
        TIRTC_LOG_DEBUG("[BIND-HTTP] reaper release begin\r\n");
        release_ret = liot_httpc_release(&ctx->client);
        if (TIRTC_ENABLE_DEBUG_LOG || release_ret != LIOT_HTTPC_SUCCESS) liot_trace("[BIND-HTTP] reaper release ret=0x%x\r\n",
...
static bool binding_http_resume_retained_cleanup(void)
{
    ...
    if (ctx->closed)
    {
        ...
        liot_rtos_task_sleep_s(BIND_HTTP_PRE_RELEASE_S);
        release_ret = liot_httpc_release(&ctx->client);
        if (TIRTC_ENABLE_DEBUG_LOG || release_ret != LIOT_HTTPC_SUCCESS) liot_trace("[BIND-HTTP] retained inline release ret=0x%x\r\n",
                   (unsigned int)release_ret);
        if (release_ret == LIOT_HTTPC_SUCCESS)
        {
            binding_http_retained_cleanup_complete(ctx);
...
#define BIND_HTTP_SETOPT_OR_FAIL(option, value)                              \
    ...
    if (!tirtc_platform_attempt_current() ||
        (expected && !demo_binding_service_context_current(expected))) return BIND_ERR_NETWORK_CHANGED;
    ...
    if (expected && !demo_binding_service_context_current(expected)) return BIND_ERR_NETWORK_CHANGED;
    ...
    if (response_capacity > BIND_SERVICE_RESPONSE_MAX) return -1;
    ctx = (binding_http_ctx_t *)liot_rtos_malloc(sizeof(*ctx)+(size_t)response_capacity);
    if (ctx == NULL)
    {
        liot_trace("[BIND-HTTP] job allocation failed\r\n");
        return -3;
    }
    memset(ctx, 0, sizeof(*ctx)+(size_t)response_capacity);
    ...
    ctx->response_capacity = response_capacity;
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] begin method=%d", method);
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] parsed port=%u scheme=%d", (unsigned int)url.port, url.scheme);
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] client=%d created\r\n", ctx->client);

    /* Only the actual SIM0 route is supported by the verified public backend.
     * The worker rejects SIM1 rather than silently making requests on SIM0. */
    ...
    if (expected && !demo_binding_service_context_current(expected)) {
        ret = BIND_ERR_NETWORK_CHANGED;
        goto cleanup;
    }
    TIRTC_LOG_DEBUG("[BIND-HTTP] perform begin\r\n");
    perform_ret = binding_http_perform(ctx);
    if (TIRTC_ENABLE_DEBUG_LOG || perform_ret != LIOT_HTTPC_SUCCESS) liot_trace("[BIND-HTTP] perform ret=0x%x\r\n",
    ...
    if (!tirtc_platform_attempt_current()) { ret = BIND_ERR_NETWORK_CHANGED; goto cleanup; }
    ...
    TIRTC_LOG_DEBUG("[BIND-HTTP] response complete status=%d bytes=%d\r\n",
...
cleanup:
    if (ctx != NULL && ctx->client != 0)
    {
        ...
        {
            TIRTC_LOG_DEBUG("[BIND-HTTP] successful cleanup deferred\r\n");
            if (stop_required)
            {
            ...
            {
                TIRTC_LOG_DEBUG("[BIND-HTTP] deferred stop skipped: closing/closed\r\n");
            }
        ...
        if (stop_required)
        {
            TIRTC_LOG_DEBUG("[BIND-HTTP] stop begin ret=%d\r\n", ret);
            stop_ret = liot_httpc_stop(&ctx->client);
        ...
        {
            TIRTC_LOG_DEBUG("[BIND-HTTP] stop skipped: closing/closed ret=%d\r\n",
                       ret);
        }
        if (!ctx->closed)
        {
        ...
        liot_rtos_task_sleep_s(BIND_HTTP_PRE_RELEASE_S);
        TIRTC_LOG_DEBUG("[BIND-HTTP] release begin\r\n");
        release_ret = liot_httpc_release(&ctx->client);
        if (TIRTC_ENABLE_DEBUG_LOG || release_ret != LIOT_HTTPC_SUCCESS) liot_trace("[BIND-HTTP] release ret=0x%x\r\n",
                   (unsigned int)release_ret);
        if (release_ret != LIOT_HTTPC_SUCCESS)
        {
            ret = -8;
        ...
        TIRTC_LOG_DEBUG("[BIND-HTTP] worker settle begin socket_fail=%u\r\n",
                   ctx->socket_open_failed ? 1U : 0U);
        liot_rtos_task_sleep_s(ctx->socket_open_failed ?
                                   BIND_HTTP_SOCKET_SETTLE_S :
                                   BIND_HTTP_NORMAL_SETTLE_S);
        TIRTC_LOG_DEBUG("[BIND-HTTP] worker settle done\r\n");
    }
    if (ctx != NULL)
    {
        memset(ctx, 0, sizeof(*ctx)+(size_t)ctx->response_capacity);
    ...
    if (TIRTC_ENABLE_DEBUG_LOG || ret != 0) liot_trace("[BIND-HTTP] end ret=%d\r\n", ret);
...
static int binding_normalize_http_base(char *url)
{
    ...
    if (length <= 7U || strncmp(url, "http://", 7U) != 0 || strpbrk(url, "\r\n\t @?#") != NULL)
    {
        return -1;
    }
    return 0;
...
static int binding_discover_services(void)
{
    ...
    if (binding_normalize_http_base(s_services.device_server) != 0 ||
        ...
        !s_services.mqtt_url[7] || strpbrk(s_services.mqtt_url + 7, "/@?#\r\n\t ") != NULL)
    {
        memset(&s_services, 0, sizeof(s_services));
        memset(response, 0, sizeof(response));
        return BIND_ERR_DISCOVERY_JSON;
...
int demo_binding_get_tirtc_identity(demo_binding_tirtc_identity_t *out)
{
    ...
    liot_rtos_exit_critical();
    if (!ready || !tirtc_platform_attempt_current())
    {
        goto cleanup;
    }
    ...
    ret = tirtc_platform_attempt_current() ? 0 : BIND_ERR_NETWORK_CHANGED;
...
{
    if (access) binding_secure_zero(access, sizeof(*access));
}
...
int demo_binding_fetch_ai_access(demo_binding_ai_access_t *out)
{
    ...
    bool ready;
    uint32_t generation, credential_epoch;
    int http_status = 0;
    int written;
    int ret = BIND_ERR_AI_NOT_READY;
    ...
    credential_epoch = s_snapshot.credential_epoch;
    ready = s_snapshot.state == DEMO_BIND_BOUND && s_snapshot.api_ready &&
            s_have_credentials &&
    ...
    liot_rtos_exit_critical();
    if (!ready || !tirtc_platform_attempt_current() || !demo_formal_mqtt_is_online() || !demo_formal_mqtt_copy_token(mqtt_token,
                                                sizeof(mqtt_token)))
    {
        goto cleanup;
    }

    /* The SDK sends this header verbatim. Reject all control/whitespace and
     * non-ASCII bytes before a token can create an additional HTTP header. */
    if (!binding_secret_valid(mqtt_token, sizeof(mqtt_token)))
    {
    ...
    if (binding_http_request_timeout_mode(request_url, LIOT_HTTPC_METHOD_GET,
                                      NULL, auth_headers, response,
                                      sizeof(response), &http_status, 15000U, true) != 0 ||
        http_status != 200)
    {
    ...
    if (!cJSON_IsNumber(business_code) ||
        ...
        binding_copy_json_string(data, "token", out->token,
                                 sizeof(out->token)) != 0 ||
        (role_id != NULL && !cJSON_IsNull(role_id) &&
         (!cJSON_IsString(role_id) || !role_id->valuestring ||
          (role_id->valuestring[0] && binding_copy_json_string(data, "role_id", out->role_id, sizeof(out->role_id)) != 0))) ||
    ...
    /* A reconnect or unbind while HTTP was in flight invalidates this grant. */
    liot_rtos_enter_critical();
    ready = s_snapshot.state == DEMO_BIND_BOUND && s_snapshot.api_ready &&
            s_snapshot.generation == generation &&
            credential_epoch != 0U && s_snapshot.credential_epoch == credential_epoch &&
            strcmp(s_credentials.device_id, credentials.device_id) == 0 &&
            strcmp(s_credentials.device_key, credentials.device_key) == 0;
    liot_rtos_exit_critical();
    if (!ready || !tirtc_platform_attempt_current() || !demo_formal_mqtt_is_online()) {
        ret = BIND_ERR_NETWORK_CHANGED;
        goto cleanup;
    }
    ...
    memcpy(out->tirtc_endpoint, services.tirtc_server,
           strlen(services.tirtc_server) + 1U);
    out->credential_epoch = credential_epoch;
...
/* Restore the upstream service transport surface with an explicit lifetime
 * lease. No feature may turn an old contact/call request into a new-account
 * operation merely because it waited behind another singleton HTTP request. */
int demo_binding_service_request_guarded(
    ...
    const demo_binding_service_context_t *expected)
{
    char base[BIND_DEVICE_SERVER_MAX] = {0};
    char token[BIND_MQTT_TOKEN_MAX] = {0};
    char headers[BIND_AI_AUTH_HEADERS_MAX] = {0};
    char url[BIND_HTTP_URL_MAX] = {0};
    const char *selected = NULL;
    uint32_t started, elapsed;
    size_t length;
    bool locked = false, valid;
    int written, result = BIND_ERR_AI_NOT_READY;
    if (!response || response_size < 2U || response_size > BIND_SERVICE_RESPONSE_MAX ||
        !http_status || !expected || !path || strncmp(path, "/v1/", 4U) != 0 ||
        timeout_ms < BIND_HTTP_WAIT_STEP_MS || timeout_ms > BIND_HTTP_TIMEOUT_MS ||
        (method != DEMO_BIND_HTTP_GET && method != DEMO_BIND_HTTP_POST)) return -1;
    response[0] = '\0';
    *http_status = 0;
    for (length = 0U; length < BIND_HTTP_URL_MAX && path[length]; ++length) {
        unsigned char c = (unsigned char)path[length];
        if (c <= 32U || c >= 127U || c == '#' || c == '\\') return -1;
    }
    if (length == BIND_HTTP_URL_MAX) return -1;
    if (json_body) {
        for (length = 0U; length < BIND_HTTP_REQUEST_MAX && json_body[length]; ++length) {}
        if (length == BIND_HTTP_REQUEST_MAX) return -1;
    }
    if ((method == DEMO_BIND_HTTP_POST && (!json_body || !json_body[0])) ||
        (method == DEMO_BIND_HTTP_GET && json_body && json_body[0])) return -1;
    if (!demo_binding_service_context_current(expected)) return BIND_ERR_NETWORK_CHANGED;
    started = liot_rtos_get_running_time();
    if (!s_http_mutex || liot_rtos_mutex_lock(s_http_mutex, timeout_ms) != LIOT_OSI_SUCCESS)
        return BIND_ERR_HTTP_MUTEX;
    locked = true;
    /* Copy endpoint and JWT in the same identity snapshot; both nested helpers
     * only copy cached RAM under the SDK's task critical section. */
    liot_rtos_enter_critical();
    valid = demo_binding_service_context_current(expected);
    if (valid) {
        switch (service) {
        case DEMO_BIND_SERVICE_DEVICE: selected = s_services.device_server; break;
        case DEMO_BIND_SERVICE_AI: selected = s_services.ai_server; break;
        case DEMO_BIND_SERVICE_VOIP: selected = s_services.voip_server; break;
        case DEMO_BIND_SERVICE_CALL: selected = s_services.call_server; break;
        default: break;
        }
        if (selected) memcpy(base, selected, sizeof(base));
        valid = selected && base[0] && demo_formal_mqtt_copy_token(token, sizeof(token));
    }
    liot_rtos_exit_critical();
    if (!valid) { result = BIND_ERR_NETWORK_CHANGED; goto cleanup; }
    if (!binding_secret_valid(token, sizeof(token))) { result = BIND_ERR_AI_JSON; goto cleanup; }
    written = snprintf(url, sizeof(url), "%s%s", base, path);
    if (written < 0 || written >= (int)sizeof(url)) { result = -1; goto cleanup; }
    written = snprintf(headers, sizeof(headers), "Authorization: Bearer %s", token);
    if (written < 0 || written >= (int)sizeof(headers)) { result = -1; goto cleanup; }
    elapsed = liot_rtos_get_running_time() - started;
    if (elapsed >= timeout_ms || timeout_ms - elapsed < BIND_HTTP_WAIT_STEP_MS) {
        result = BIND_ERR_HTTP_MUTEX; goto cleanup;
    }
    if (!demo_binding_service_context_current(expected)) {
        result = BIND_ERR_NETWORK_CHANGED; goto cleanup;
    }
    result = binding_http_request_unlocked(url,
        method == DEMO_BIND_HTTP_POST ? LIOT_HTTPC_METHOD_POST : LIOT_HTTPC_METHOD_GET,
        json_body, headers, response, (int)response_size, http_status,
        timeout_ms - elapsed, true, expected);
    if (!demo_binding_service_context_current(expected)) result = BIND_ERR_NETWORK_CHANGED;
cleanup:
    if (locked) (void)liot_rtos_mutex_unlock(s_http_mutex);
    if (result) { binding_secure_zero(response, response_size); *http_status = 0; }
    binding_secure_zero(base, sizeof(base));
    binding_secure_zero(token, sizeof(token));
    binding_secure_zero(headers, sizeof(headers));
    binding_secure_zero(url, sizeof(url));
    return result;
}
...
static int binding_report_device(bool signed_request)
{
    ...
    if (!cJSON_IsNumber(business_code) || business_code->valueint != 200 ||
        ...
        !binding_identifier_valid(s_report.temp_client_id, sizeof(s_report.temp_client_id)) ||
        !binding_secret_valid(s_report.temp_token, sizeof(s_report.temp_token)))
    {
...
{
    liot_rtos_enter_critical();
    s_api_ready = false; s_snapshot.api_ready = false;
    liot_rtos_exit_critical();
    char token_url[256];
    char auth_headers[BIND_AUTH_HEADERS_MAX];
    char response[BIND_HTTP_RESPONSE_MAX];
    ...
    if (token_len >= mqtt_token_out_size || !binding_secret_valid(mqtt_token->valuestring, mqtt_token_out_size))
    {
...
/* F6D_A passes a pointer to a stack-local copy of the client handle.
 * Match the handle VALUE and the connection generation, never its address.
 * The caller keeps teardown from reusing this handle while a callback runs. */
static bool binding_mqtt_callback_matches(liot_mqtt_client_t *client, void *arg)
{
    return client != NULL && s_mqtt_client != 0 &&
           *client == s_mqtt_client && (uintptr_t)arg == s_temp_cookie;
}

static void binding_mqtt_event_body(liot_mqtt_client_t *client,
                                  int event,
                                  void *arg,
                                  void *data)
{
    ...
    if (!binding_mqtt_callback_matches(client, arg)) return;
    if ((event == LIOT_MQTT_CONNECT_EVENT || event == LIOT_MQTT_SUB_EVENT || event == LIOT_MQTT_PUB_EVENT) &&
        !tirtc_platform_attempt_current()) return;
    switch (event)
    {
    ...
    case LIOT_MQTT_SUB_EVENT:
        /* F6D_A reports completion result: 0 succeeds, -1 fails. */
        liot_trace("[BIND-MQTT] SUBACK result=%d\r\n", value);
        s_mqtt_sub_ok = (value == 0);
        s_mqtt_sub_done = true;
        binding_mqtt_signal();
        break;

    case LIOT_MQTT_PUB_EVENT:
        liot_trace("[BIND-MQTT] PUBACK result=%d\r\n", value);
        s_mqtt_ack_ok = (value == 0);
...
static void binding_mqtt_event_cb(liot_mqtt_client_t *client, int event, void *arg, void *data)
{
    bool accepted;
    liot_rtos_enter_critical();
    accepted = s_temp_events_allowed && binding_mqtt_callback_matches(client, arg);
    if (accepted) s_temp_callbacks++;
    liot_rtos_exit_critical();
    if (!accepted) return;
    binding_mqtt_event_body(client, event, arg, data);
    liot_rtos_enter_critical(); s_temp_callbacks--; liot_rtos_exit_critical();
}

static void binding_mqtt_exception_cb(liot_mqtt_client_t *client)
{
    /* This SDK callback has no generation argument and reuses handle storage.
     * It cannot identify its connection. The owner observes real state/events
     * every 250 ms with a bounded deadline; a late exception changes nothing. */
    (void)client;
}

static void binding_mqtt_incoming_body(liot_mqtt_client_t *client,
                                     void *arg,
                                     int packet_id,
                                     const char *topic,
                                     ...
                                     unsigned short payload_len)
{
    ...
    (void)client;
    (void)packet_id;
    if (!s_temp_accept || (uintptr_t)arg != s_temp_cookie || !tirtc_platform_attempt_current()) return;
    ...
    TIRTC_LOG_DEBUG("[BIND-MQTT] command received bytes=%u\r\n",
    ...
    if (!binding_identifier_valid(device_id->valuestring, sizeof(s_grant_device_id)) ||
        !binding_secret_valid(device_key->valuestring, sizeof(s_grant_device_key)) ||
        id_len >= sizeof(s_grant_device_id) || key_len >= sizeof(s_grant_device_key))
    {
        s_grant_invalid = true;
        cJSON_Delete(root);
        binding_mqtt_signal();
...
static void binding_mqtt_incoming_cb(liot_mqtt_client_t *client, void *arg,
                                     int packet_id, const char *topic,
                                     const unsigned char *payload, unsigned short payload_len)
{
    bool accepted;
    liot_rtos_enter_critical();
    accepted = s_temp_accept && binding_mqtt_callback_matches(client, arg);
    if (accepted) s_temp_callbacks++;
    liot_rtos_exit_critical();
    if (!accepted) return;
    binding_mqtt_incoming_body(client, arg, packet_id, topic, payload, payload_len);
    liot_rtos_enter_critical(); s_temp_callbacks--; liot_rtos_exit_critical();
}

static bool binding_mqtt_callbacks_drained(void)
{
    uint32_t started = liot_rtos_get_running_time();
    uint32_t active;
    do {
        liot_rtos_enter_critical(); active = s_temp_callbacks; liot_rtos_exit_critical();
        if (!active) return true;
        liot_rtos_task_sleep_ms(10U);
    } while ((uint32_t)(liot_rtos_get_running_time() - started) < 2000U);
    return false;
}
...
{
    liot_rtos_enter_critical(); s_temp_accept = false; liot_rtos_exit_critical();
    if (!binding_mqtt_callbacks_drained()) return LIOT_MQTTCLIENT_WOUNDBLOCK;
    ...
    if (s_mqtt_client != 0)
    {
        /* Accepted deinit is submitted exactly once. Its SDK worker may take
         * longer than this attempt; retain every referenced object meanwhile. */
        if (s_mqtt_deinit_submitted) goto await_deinit;
        state = liot_mqtt_client_state(&s_mqtt_client);
        TIRTC_LOG_DEBUG("[BIND-MQTT] cleanup begin state=%d\r\n", state);
        s_mqtt_disconnect_done = false;
        ...
        state = liot_mqtt_client_state(&s_mqtt_client);
        TIRTC_LOG_DEBUG("[BIND-MQTT] disconnect settled state=%d done=%d\r\n",
        ...
        /* Allow DISCONNECT to settle. A queued deinit is not completion. */
        TIRTC_LOG_DEBUG("[BIND-MQTT] settle before deinit %u ms\r\n",
                   (unsigned int)BIND_MQTT_SETTLE_MS);
        liot_rtos_task_sleep_ms(BIND_MQTT_SETTLE_MS);
        liot_rtos_enter_critical(); s_temp_events_allowed = false; liot_rtos_exit_critical();
        if (!binding_mqtt_callbacks_drained()) { s_mqtt_cleanup_pending = true; return LIOT_MQTTCLIENT_WOUNDBLOCK; }
        TIRTC_LOG_DEBUG("[BIND-MQTT] deinit begin\r\n");
        ret = liot_mqtt_client_deinit(&s_mqtt_client);
        liot_trace("[BIND-MQTT] deinit submit ret=%d\r\n", ret);
        if (ret != LIOT_MQTTCLIENT_SUCCESS && ret != LIOT_MQTTCLIENT_WOUNDBLOCK)
        {
        ...
        s_mqtt_deinit_submitted = true;
await_deinit:
        s_mqtt_cleanup_pending = true;
        timeout = liot_rtos_get_running_time() + BIND_MQTT_ASYNC_DEINIT_MS;
        while (!tirtc_mqtt_cleanup_complete(&s_mqtt_client)) {
            if ((int32_t)(timeout - liot_rtos_get_running_time()) <= 0) return LIOT_MQTTCLIENT_WOUNDBLOCK;
            liot_rtos_task_sleep_ms(100U);
        }
        s_mqtt_client = 0;
        s_mqtt_deinit_submitted = false;
    ...
    {
        old_sem = s_mqtt_sem;
        if (tirtc_platform_sem_delete_idle(old_sem) != LIOT_OSI_SUCCESS)
        {
    ...
    memset(s_report.temp_token, 0, sizeof(s_report.temp_token));
    memset(&s_temp_options, 0, sizeof(s_temp_options));
    memset(s_grant_device_key, 0, sizeof(s_grant_device_key));
    s_mqtt_cleanup_pending = false;
...
static int binding_wait_mqtt_connected(uint32_t deadline)
{
    ...
    while (tirtc_platform_attempt_current() && binding_deadline_pending(timeout) &&
...
static int binding_wait_subscribed(uint32_t deadline)
{
           ...
           binding_deadline_pending(deadline))
    {
        ...
        if (s_mqtt_sub_done)
        {
            return s_mqtt_sub_ok ? 0 : -1;
        }
        binding_snapshot_set_seconds(binding_seconds_remaining(deadline));
        liot_rtos_semaphore_wait(s_mqtt_sem, 250);
    }
    liot_trace("[BIND-MQTT] subscribe wait ended done=%d ok=%d lost=%d current=%d state=%d\r\n",
               s_mqtt_sub_done, s_mqtt_sub_ok, s_mqtt_lost,
               tirtc_platform_attempt_current(), liot_mqtt_client_state(&s_mqtt_client));
    return -1;
}
...
static int binding_send_grant_ack(void)
{
    ...
    ret = liot_mqtt_publish(&s_mqtt_client, ack_topic, ack,
                            ...
                            NULL, NULL);
    liot_trace("[BIND-MQTT] grant ACK submit ret=%d\r\n", ret);
    if (ret != LIOT_MQTTCLIENT_SUCCESS && ret != LIOT_MQTTCLIENT_WOUNDBLOCK)
    {
        return -1;
    }
...
{
    liot_mqtt_client_option *options = &s_temp_options;
    const char *log_broker;
    int ret;

    /* The SDK's six clients share a static pool. A retired formal client may
     * look free before every task/queue has exited; no other init may race it. */
    if (demo_formal_mqtt_prepare_binding() != 0) return BIND_ERR_MQTT_INIT;
    ...
    s_temp_cookie++; if (!s_temp_cookie) s_temp_cookie++;
    s_temp_accept = true; s_temp_events_allowed = true;
    ...
    if (liot_mqtt_client_init_ex(&s_mqtt_client, BIND_PDP_CID,
                                 binding_mqtt_event_cb, (void *)(uintptr_t)s_temp_cookie) !=
    ...
    ret = liot_mqtt_set_inpub_callback(&s_mqtt_client,
                                       binding_mqtt_incoming_cb, (void *)(uintptr_t)s_temp_cookie);
    if (ret != LIOT_MQTTCLIENT_SUCCESS)
    {
    ...
    ret = liot_mqtt_sub_unsub(&s_mqtt_client, s_cmd_topic, 1,
                              NULL, NULL, 1);
    liot_trace("[BIND-MQTT] subscribe submit ret=%d state=%d\r\n",
               ret, liot_mqtt_client_state(&s_mqtt_client));
    if (ret == LIOT_MQTTCLIENT_SUCCESS || ret == LIOT_MQTTCLIENT_WOUNDBLOCK)
    {
    ...
    while (tirtc_platform_attempt_current() && binding_deadline_pending(deadline))
    {
        binding_snapshot_set_seconds(binding_seconds_remaining(deadline));
        if (s_grant_invalid)
        {
        ...
        if (s_grant_ready)
        {
            if (!tirtc_platform_attempt_current()) { (void)binding_mqtt_cleanup(); return BIND_ERR_NETWORK_CHANGED; }
        ...
        if (s_mqtt_lost || liot_mqtt_client_state(&s_mqtt_client) != MQTT_CONN_CONNECTED)
        {
            (void)binding_mqtt_cleanup();
            return BIND_ERR_MQTT_CONNECT;
        }
                                     ...
                                     size_t mqtt_token_size)
{
    ...
    if (!tirtc_platform_attempt_current()) return BIND_ERR_NETWORK_CHANGED;
    ...
    if (!tirtc_platform_attempt_current()) { demo_formal_mqtt_disconnect(); return BIND_ERR_NETWORK_CHANGED; }
...
{
    s_binding_required = true; s_api_ready = false;
    binding_snapshot_set(DEMO_BIND_VERIFYING, NULL, 0, BIND_ERR_NONE);
    demo_formal_mqtt_disconnect();
    /* Preserve identity; the next signed /token then /report proves rebind. */
...
void demo_binding_task(void *argv)
{
    ...
    s_binding_required = !s_have_credentials;
    ...
    while (1)
    {
        ...
        {
            if (!tirtc_platform_attempt_current()) {
                s_api_ready = false; demo_formal_mqtt_disconnect();
                binding_snapshot_set(DEMO_BIND_ERROR, NULL, 0, BIND_ERR_NETWORK_CHANGED);
                demo_binding_request(); continue;
            }
        ...
        binding_snapshot_set(DEMO_BIND_WAIT_NETWORK, NULL, 0,
                             BIND_ERR_NONE);
        while (!tirtc_network_is_ready() ||
               !tirtc_network_time_ready())
        {
            liot_rtos_task_sleep_s(1);
        }

        { tirtc_network_snapshot_t next_attempt;
          tirtc_network_get_snapshot(&next_attempt);
          liot_rtos_enter_critical(); s_attempt = next_attempt;
          s_attempt_valid = s_attempt.ready && s_attempt.time_valid;
          liot_rtos_exit_critical(); }
        if (!s_attempt_valid) { binding_snapshot_set(DEMO_BIND_ERROR, NULL, 0, BIND_ERR_NETWORK_CHANGED); demo_binding_request(); continue; }
        if (s_attempt.sim_id != 0) {
            s_attempt_valid = false;
            binding_snapshot_set(DEMO_BIND_ERROR, NULL, 0, BIND_ERR_SIM_UNSUPPORTED);
            continue;
        }
        if (demo_formal_mqtt_prepare_binding() != 0) {
            binding_snapshot_set(DEMO_BIND_ERROR, NULL, 0, BIND_ERR_FORMAL_MQTT);
            continue;
        }
        ...
        if (ret != 0)
        {
            if (!s_mqtt_cleanup_pending && !s_mqtt_client) memset(&s_report, 0, sizeof(s_report));
            binding_snapshot_set(DEMO_BIND_ERROR, NULL, 0, ret);
            continue;
        }
