...
| LIVE 远端对讲 | 网页远端接入；仅设备在主页且空闲时允许，离开主页后结束。 | [操作说明（第 10 章）](docs/当前设备操作.md) |
...
仓库 `firmware/lite/latest.binpkg` 提供 lite 最新完整烧录包，包含原有 AI 呼叫联系人、GROUP 多人对讲及应用异常防护，可按第 4 节下载烧录。本次构建的应用 `.bin` 与迁移前逐字节一致，未重新烧录实机；固件来源、校验值与验证范围见[应用说明](docs/应用说明.md#当前固件与验证)。历史 `0.1.0` 固件不包含 GROUP 与这些异常防护更新。

首次体验这两项功能：

- **AI 呼叫**：先确认 `WX`/`DEV` 手动呼叫可用，再给设备智能体启用 `call_contact` 插件、保存并绑定；进入 `AI` 说出联系人完整名称。逐项填写和示例图见[AI 呼叫配置](docs/AI呼叫微信与设备配置详解.md)。
- **多人对讲**：网页上由设备 A 创建房间，设备 B 用同一六位房间号加入；两台设备分别长按返回进入 `GROUP`，等待 `LISTENING` 后用确定键开麦试音。网页逐步操作和退出方法见[多人对讲操作](docs/当前设备操作.md#10a-独立多人对讲group)。此功能不需要 AI 插件或 WX/DEV 联系人关系。

### 操作视频

点击标题展开观看或下载。若当前仓库页面不显示播放器，点击“下载原视频”即可；全部原视频随本仓库保存。
...
![通过 AI 对话呼叫微信和其他设备](../../../assets/videos/ai-call-contacts.mp4)

[下载原视频](../../../assets/videos/ai-call-contacts.mp4) · [配置与操作步骤](docs/AI呼叫微信与设备配置详解.md)
...
![创建房间多人对讲](../../../assets/videos/group-intercom.mp4)

[下载原视频](../../../assets/videos/group-intercom.mp4) · [网页建房、加入与按键操作](docs/当前设备操作.md#10a-独立多人对讲group)
...
1. **下载固件**：[下载 lite 最新固件（约 2.94 MiB）](../../../firmware/lite/latest.binpkg)。已包含底包、应用及多人对讲功能，只需这一个 `.binpkg`，适用于上述硬件配置。先核对 [SHA-256](../../../firmware/lite/SHA256SUMS.txt) 与 [验证记录](docs/应用说明.md#当前固件与验证)；本次尚未烧录实机。
...
4. **开始对讲**：按 `KEY0` 选择、`KEY1` 确认、`KEY2` 返回。使用 AI 前先配置并绑定平台智能体；使用 WX 时，在微信搜索“探鸽小钛”，登录同一账号并完成设备的 VoIP 授权与上报。详细步骤见[当前设备操作](docs/当前设备操作.md)。

**全量烧录可能清除绑定及文件系统数据，请先备份，烧录期间不要断电。**

## 5. 从源码接入与二次开发

先按 [源码准备](../../../README.md#编译与下载)获取完整仓库；首次编译会自动解压自带工具链。本仓库已包含 lite 应用及其编译所需的利尔达 SDK、F6D_A 底包和工具。lite 的业务、配置及 TiRTC 库位于独立的 `tirtc_lite` 目录，不引用 pro 的业务代码。在仓库根目录执行 Windows 编译命令：

```powershell
.\build_tirtc.bat lite
```

编译成功后自动更新 `firmware/lite/latest.binpkg` 和同目录的 `SHA256SUMS.txt`，烧录方法同上。`gccout/tirtc_lite` 保存编译中间产物，不随仓库发布；`firmware/tirtc_lite/` 中的命名包保留作迁移验证记录，不随这条命令更新。修改功能或日志开关后，应使用新的构建输出目录，示例见[应用构建说明](docs/应用说明.md#默认构建)。

本脚本选择 `APP_VARIANT=lite BUILD_MODE=demo DEMO_NAME=tirtc_app`，并使用独立输出目录；旧工程不传 `APP_VARIANT` 的默认命令继续选择 lite。

若使用利尔达工具从源码编译，选择以下参数（路径相对于仓库根目录）：

| 配置项 | 选择 |
| --- | --- |
| 应用文件夹 | `examples/L_CT4IT00_YP00W_01_V04`（不是下一级 `tirtc_lite` 目录） |
| 底包路径 | `components/basePkg/F6D_A` |
| 模组型号 | `NT26F6D0` |

lite 应用位于 [examples/L_CT4IT00_YP00W_01_V04/tirtc_lite](.)：利尔达 SDK 提供 4G、系统和硬件能力，应用完成绑定与呼叫信令，TiRTC 负责实时连接及音频、命令传输。

- [当前设备操作](docs/当前设备操作.md)：绑定、按键、AI 呼叫、网页创建/加入多人房间及 OLED 状态。
- [AI 呼叫微信与设备配置](docs/AI呼叫微信与设备配置详解.md)：联系人准备、智能体和设备插件填写示例。
- [代码与流程详解](docs/代码与流程详解.md)：文件职责、模块架构、AI 转呼和多人对讲业务流程。
- [TiRTC 接口调用与模块流程](docs/TiRTC接口调用与模块流程.md)：SDK 调用位置、参数与回调，以及 AI 呼叫命令和多人房间信令。
